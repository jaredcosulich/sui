...
pub trait RPCGateway {
    ...
    #[method(name = "objects")]
    async fn objects(&self, owner: SuiAddress) -> RpcResult<ObjectResponse>;

    #[method(name = "object_info")]
    async fn object_info(&self, object_id: ObjectID) -> RpcResult<ObjectRead>;
...
async fn run_server() -> anyhow::Result<(SocketAddr, HttpServerHandle)> {
    ...
    let server_handle = server.start(RPCGatewayImpl::new(&config_path)?.into_rpc())?;
